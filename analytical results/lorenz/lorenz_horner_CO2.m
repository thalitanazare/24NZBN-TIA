...
for k = 2:1:2
    expr_x = 'x(k-1)*(0.85919-0.2833*10^(-2)*z(k-1)-0.69159*10^(-3)*y(k-1)+0.73257*10^(-3)*x(k-1))+y(k-1)*(0.22489-0.34598*10^(-3)*z(k-1))-0.12927*10^(-1)';
    expr_y = 'z(k-1)*(-0.6713*10^(-2)*y(k-1)-0.18688*10^(-1)*x(k-1)-0.33705*10^(-4)*z(k-1))+x(k-1)*(0.54947+0.67054*10^(-4)*y(k-1)-0.36965*10^(-3)*x(k-1))+1.109*y(k-1)';
    expr_z = 'y(k-1)*(0.93353*10^(-2)*x(k-1)+0.76919*10^(-2)*y(k-1)+0.48760*10^(-4)*z(k-1))+z(k-1)*(1.0077-0.21708*10^(-2)*z(k-1))+0.72469*10^(-2)*x(k-1)^2-0.47834';
